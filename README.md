...
- `QTICKETS_API_TOKEN` — токен доступа к API Qtickets (в [личном кабинете](https://qtickets.app/organizers/myaccount), раздел «Настройки → Основное»). Без него процесс **запустится**, но обработка вебхука вернёт `503`, пока токен не задан.
